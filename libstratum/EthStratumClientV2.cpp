	...
	else
	{
		...
		switch (m_protocol) {
			...
			case STRATUM_PROTOCOL_ETHPROXY:
				...
				if (m_email.empty())
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"vns_submitLogin\", \"params\": [\"" << user << "\"]}\n";
				}
				else
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"vns_submitLogin\", \"params\": [\"" << user << "\", \"" << m_email << "\"]}\n";
				}
				break;
...
bool EthStratumClientV2::submitHashrate(string const & rate) {
	...
	string json = "{\"id\": 6, \"jsonrpc\":\"2.0\", \"method\": \"vns_submitHashrate\", \"params\": [\"" + rate + "\",\"0x" + this->m_submit_hashrate_id + "\"]}\n";
	std::ostream os(&m_requestBuffer);
	os << json;
	write(m_socket, m_requestBuffer);
	return true;
...
bool EthStratumClientV2::submit(Solution solution) {
	...
	case STRATUM_PROTOCOL_STRATUM:
		...
		break;
	case STRATUM_PROTOCOL_ETHPROXY:
		json = "{\"id\": 4, \"worker\":\"" + m_worker + "\", \"method\": \"vns_submitWork\", \"params\": [\"0x" + nonceHex + "\",\"0x" + solution.headerHash.hex() + "\",\"0x" + solution.mixHash.hex() + "\"]}\n";
		break;
	case STRATUM_PROTOCOL_ETHEREUMSTRATUM:
