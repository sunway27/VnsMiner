		...
		while (m_running)
			try
			{
				...
				for (unsigned i = 0; !completed; ++i)
				{
					...
					try
					{
						prpc->vns_submitHashrate(toJS(rate), "0x" + id.hex());
					}
				...
				bool ok = prpc->vns_submitWork("0x" + toHex(solution.nonce), "0x" + toString(solution.headerHash), "0x" + toString(solution.mixHash));
				if (ok) {
